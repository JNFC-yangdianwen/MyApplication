...
public class CitiesActivity extends BaseActivity implements OnWheelChangedListener {
	...
	private void updateAreas() {
		...
		mCurrentCityName = mCitisDatasMap.get(mCurrentProviceName)[pCurrent];
		 if(mCurrentProviceName.contains("北京")||mCurrentProviceName.contains("天津")||mCurrentProviceName.contains("上海")||mCurrentProviceName.contains("重庆")){
    	 }else{
	...
	public void showChoose(View view) {
		// Toast.makeText(this, mCurrentProviceName+"  " + mCurrentCityName
		// +"  " + mCurrentAreaName, 1).show();
		// // 发送信息
		EventBus.getDefault().post(new EvenBusBean("city", mCurrentProviceName, mCurrentCityName,mCurrentAreaName));
		// 设置地址
		SharedPreferences adrrsp = getApplicationContext().getSharedPreferences("useradrr", 0);
		Editor edit = adrrsp.edit();
		edit.putString("province", mCurrentProviceName);
		edit.putString("city", mCurrentCityName);
		edit.putString("dist", mCurrentAreaName);
		edit.commit();
		this.finish();
	}
}
